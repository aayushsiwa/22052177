...
function Feed() {
    ...
    const handleInitialData = useCallback((data: { id: number; userId: number; username: string; createdAt: string; }[]) => {
    ...
    const reconnect = useCallback(() => {
        ...
        if ((window as { feedDisconnect?: () => void }).feedDisconnect) {
            (window as { feedDisconnect?: () => void }).feedDisconnect?.();
        }
    ...
    }, [handleInitialData, handleError, pollingInterval]);

    useEffect(() => {
        (window as { feedDisconnect?: () => void }).feedDisconnect = api.connectToFeed(
            handleInitialData,
            handleError,
            pollingInterval
        );
