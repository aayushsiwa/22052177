...
const TrendingPosts = () => {
    ...
    useEffect(() => {
        const fetchTrendingPosts = async () => {
            try {
                setLoading(true);
                const trendingPosts: Array<Post & { [key: string]: unknown }> = await api.getTrendingPosts();
                const uniquePosts = Array.from(
                    new Map(
                        trendingPosts.map((post) => [post.id, post])
                    ).values()
                );
                ...
                setError(
                    `Failed to load trending posts. ${(err as Error).message}`
                );
                setLoading(false);
            }
        };
                            ...
                            <div className="bg-gradient-to-r from-purple-600 to-blue-500 text-white p-4 rounded-lg mb-6">
                                <div className="flex items-center">
                                    ...
                                    <div>
                                        ...
                                        <p>
                                            These posts are generating the most
                                            conversations
                                        </p>
                                    </div>
                                </div>
